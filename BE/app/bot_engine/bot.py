...
from app.bot_engine.grid_runtime import cancel_all_bot_orders, sync_bot_orders, tick_grid_bot
...
def stop_grid_bot_once(db, bot: TradingBot, current_user: User, *, cancel_open: bool = True) -> TradingBot:
    ...
    should_cancel = bool(bot.settings.get(
        "cancel_orders_on_stop", True)) if bot.settings else True
...
def sync_grid_bot_orders(db, bot: TradingBot, current_user: User) -> list[TradingBotOrder]:
    ...
    records = sync_bot_orders(db, bot)
