...
const SELECT_OPTIONS = {
  exchange: [{ value: "bybit", label: "Bybit" }],
  environment: [
    ...
    <main className={styles.bots}>
      <div className="container">
        <header className={styles.hero}>
          ...
          <p className="lead">
            Кожен бот зберігається в базі даних окремо для вашого акаунта. Тут
            лише налаштування: жодного реального запуску торгів з інтерфейсу
            поки немає.
          </p>
            ...
            {!loading && bots.length === 0 ? (
              <Card className={styles.emptyState}>
                ...
                <p className="text-secondary">
                  Створіть перший grid-бот для demo, testnet або live
                  середовища.
                </p>
                <Button icon={<Plus size={16} />} onClick={openCreate}>
                  Створити першого бота
                </Button>
              </Card>
            ) : null}
                ...
                {bots.map((bot) => (
                  <Card key={bot.id} className={styles.botCard}>
                    <CardHeader
                      ...
                      action={
                        <span
                          className={`${styles.status} ${bot.is_active ? styles.statusActive : styles.statusPaused}`}
                        >
                    ...
                    <dl className={styles.botSpecs}>
                      <div>
                        <dt>Symbol</dt>
                        <dd className="mono">{bot.symbol}</dd>
                      </div>
                      <div>
                        <dt>Order Qty</dt>
                        <dd className="mono">{bot.order_qty}</dd>
                      </div>
                      <div>
                        <dt>Grid Orders</dt>
                        <dd className="mono">{bot.grid_orders_count}</dd>
                      </div>
                      <div>
                        <dt>Grid Step %</dt>
                        <dd className="mono">{bot.grid_step_percent}</dd>
                      </div>
                      <div>
                        <dt>Runtime</dt>
                        <dd className="mono">{bot.runtime_status}</dd>
                      </div>
                      <div>
                        <dt>Enabled</dt>
                        <dd className="mono">{bot.is_active ? "Yes" : "No"}</dd>
                      </div>
                    </dl>
                    ...
                    <div className={styles.botActions}>
                      ...
                      <Link to={`/bots/${bot.id}`} className={styles.botLink}>
                        Open
                      </Link>
                      <Button
                        variant="danger"
            ...
            <Card className={styles.formCard}>
              ...
              {!isFormOpen ? (
                <div className={styles.formPlaceholder}>
                  <p className="text-secondary">
                    Оберіть існуючого бота для редагування або створіть нового.
                  </p>
                  <Button icon={<Plus size={16} />} onClick={openCreate}>
                    Відкрити форму
                  </Button>
                ...
                <form className={styles.form} onSubmit={handleSubmit}>
                  ...
                  <div className={styles.fieldGrid}>
                    <label className={styles.field}>
                      ...
                      <select
                        ...
                        onChange={handleChange}
                      >
                        {SELECT_OPTIONS.exchange.map((option) => (
                    ...
                    <label className={styles.field}>
                      ...
                      <select
                        ...
                        onChange={handleChange}
                      >
                        {SELECT_OPTIONS.environment.map((option) => (
                          <option key={option.value} value={option.value}>
                            {option.label}
                          </option>
                        ))}
                  ...
                  <div className={styles.fieldGrid}>
                    <label className={styles.field}>
                      ...
                      <select
                        ...
                        onChange={handleChange}
                      >
                        {SELECT_OPTIONS.strategy_type.map((option) => (
                          <option key={option.value} value={option.value}>
                            {option.label}
                          </option>
                        ))}
                    ...
                    <label className={styles.field}>
                      ...
                      <select
                        ...
                        onChange={handleChange}
                      >
                        {SELECT_OPTIONS.category.map((option) => (
                          <option key={option.value} value={option.value}>
                            {option.label}
                          </option>
                        ))}
                  ...
                  <div className={styles.fieldGrid}>
                    <label className={styles.field}>
                      <span>Grid Orders Count</span>
                      <input
                        name="grid_orders_count"
                        type="number"
                        min="1"
                        step="1"
                        value={form.grid_orders_count}
                        onChange={handleChange}
                        required
                      />
                    </label>
                    <label className={styles.field}>
                      <span>Grid Step %</span>
                      <input
                        name="grid_step_percent"
                        type="number"
                        min="0.01"
                        step="0.01"
                        value={form.grid_step_percent}
                        onChange={handleChange}
                        required
                      />
                    </label>
                  ...
                  <div className={styles.formActions}>
                    <Button type="submit" loading={saving}>
                      {editingBotId ? "Зберегти зміни" : "Створити бота"}
                    </Button>
