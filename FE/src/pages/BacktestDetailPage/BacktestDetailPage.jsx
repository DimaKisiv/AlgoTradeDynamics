...
const TAB_LABELS = {
  uk: { Summary: "Підсумок", Chart: "Графік", Cycles: "Цикли", Orders: "Ордери", Executions: "Виконання", Positions: "Позиції", Events: "Події", Configuration: "Конфігурація" },
  en: { Summary: "Summary", Chart: "Chart", Cycles: "Cycles", Orders: "Orders", Executions: "Executions", Positions: "Positions", Events: "Events", Configuration: "Configuration" },
};
const ORDER_FILTER_LABELS = {
  uk: { all: "усі", open: "відкриті", filled: "виконані", cancelled: "скасовані", rejected: "відхилені" },
  en: { all: "all", open: "open", filled: "filled", cancelled: "cancelled", rejected: "rejected" },
};
const EVENT_FILTER_LABELS = {
  uk: { important: "важливі", all: "усі", risk: "ризик", errors: "помилки" },
...
function Empty({ children }) {
  ...
  return <div className={styles.empty}>{children ?? tr("Немає даних", "No data")}</div>;
}
...
function PriceTooltip({ active, payload }) {
  ...
  return <div className={styles.tooltip}>
    <strong>{new Date(timestamp).toLocaleString(locale)}</strong>
    {row.open != null && <>
      <span>{tr("Відкриття", "Open")} {fmtMoney(row.open)} · {tr("Максимум", "High")} {fmtMoney(row.high)}</span>
      <span>{tr("Мінімум", "Low")} {fmtMoney(row.low)} · {tr("Закриття", "Close")} {fmtMoney(row.close)}</span>
      <span>{tr("Equity", "Equity")} {fmtMoney(row.equity)} · {tr("Доступно", "Available")} {fmtMoney(row.available_balance)}</span>
      <span>{tr("Позиція", "Position")} {fmtNumber(row.position_qty, 6)} · {tr("Просадка", "Drawdown")} {formatAxisPercent(row.drawdown_percent)}</span>
      <span>{tr("Відкритий PnL", "Open PnL")} {fmtMoneySigned(row.unrealized_pnl)}</span>
    </>}
    {markerPayload && <>
...
function OrderMarker({ cx, cy, payload, onSelect }) {
  ...
  const title = `${payload.label} · ${fmtMoney(payload.price)} · ${fmtNumber(payload.qty, 6)} · ${new Date(payload.timestamp).toLocaleString(locale)}`;
  const commonProps = {
  ...
  return <div className={styles.miniChart}>
    ...
    <ResponsiveContainer width="100%" height={210}>
      <AreaChart data={data} syncId={CHART_SYNC_ID} syncMethod="value" margin={{ top: 8, right: 10, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="timestamp" type="number" scale="time" domain={timeDomain} ticks={timeTicks} allowDataOverflow height={28} tickFormatter={(v) => formatChartTime(v, spanMs, locale)} stroke="#68718a" tick={{ fontSize: 10 }}/>
...
export default function BacktestDetailPage() {
  ...
  const displayMarkers = useMemo(() => {
    ...
    return [...groups.values()].map((group) => {
      ...
      return {
        ...
        compact: true,
        clusterItems: group,
        label: first.kind === "cancelled" ? tr(`${group.length} скасованих ордерів`, `${group.length} cancelled orders`) : isTp ? tr(`${group.length} виконань take-profit`, `${group.length} take-profit fills`) : tr(`${group.length} виконань входу`, `${group.length} entry fills`),
      };
    });
  }, [visibleMarkers, filteredPoints, chartSpanMs, tr]);
  ...
  }, [orderedExecutions, orders, run?.bot_snapshot?.strategy_type]);
  ...
  return (
    <main className={styles.page}>
      <div className="container">
        <Link to="/backtests" className={styles.back}><ArrowLeft size={15}/> {tr("Backtests", "Backtests")}</Link>
        <header className={styles.hero}>
          ...
          <div className={styles.controls}>
            ...
            {ACTIVE.has(run.status) && <Button variant="danger" icon={<OctagonX size={16}/>} disabled={busy} onClick={() => control(() => backtestsApi.cancel(id))}>{tr("Скасувати", "Cancel")}</Button>}
          </div>
        </header>
        {error && <div className={styles.error}>{error}</div>}
        {ACTIVE.has(run.status) && <div className={styles.liveBar}><div className={styles.progress}><i style={{width:`${run.progress}%`}}/></div><div><span>{tr("Симульовано:", "Simulated:")} {run.current_time ? new Date(run.current_time).toLocaleString(locale) : tr("Підготовка…", "Preparing…")}</span><span>{run.processed_candles.toLocaleString(locale)} / {run.total_candles.toLocaleString(locale)} {tr("свічок", "candles")}</span><span>{run.current_price ? fmtMoney(run.current_price) : "—"}</span></div></div>}
        {run.error && <div className={styles.error}>{run.error}</div>}

        <nav className={styles.tabs}>{TABS.map((item) => <button key={item} className={tab === item ? styles.activeTab : ""} onClick={() => setTab(item)}>{TAB_LABELS[language]?.[item] || item}</button>)}</nav>

        {tab === "Summary" && <>
          <section className={styles.heroMetrics}>
            <Metric label={tr("Чистий загальний PnL", "Net Total PnL")} value={metrics.net_total_pnl == null ? "—" : fmtMoneySigned(metrics.net_total_pnl)} tone={asNumber(metrics.net_total_pnl)>=0?"positive":"negative"}/>
            <Metric label={tr("Дохідність", "Return")} value={metrics.return_percent == null ? "—" : fmtPctSigned(metrics.return_percent)} tone={asNumber(metrics.return_percent)>=0?"positive":"negative"}/>
            <Metric label={tr("Кінцевий equity", "Final Equity")} value={metrics.final_equity == null ? "—" : fmtMoney(metrics.final_equity)}/>
            <Metric label={tr("Максимальна просадка", "Maximum Drawdown")} value={metrics.maximum_drawdown_percent == null ? "—" : fmtPctSigned(metrics.maximum_drawdown_percent)} tone="negative"/>
          </section>
          <div className={styles.summaryGrid}>
            <section className={styles.panel}><h2>{tr("Прибутковість", "Profitability")}</h2><div className={styles.metricGrid}>
              <Metric label={tr("Валовий реалізований", "Gross realized")} value={fmtMoneySigned(metrics.gross_realized_pnl)}/><Metric label={tr("Чистий реалізований", "Net realized")} value={fmtMoneySigned(metrics.net_realized_pnl)}/><Metric label={tr("Відкритий нереалізований", "Open unrealized")} value={fmtMoneySigned(metrics.unrealized_pnl)}/><Metric label={tr("Загальні комісії", "Total fees")} value={fmtMoney(metrics.total_fees)}/><Metric label={tr("Середній цикл", "Average cycle")} value={fmtMoneySigned(metrics.average_cycle_pnl)}/><Metric label={tr("Найкращий цикл", "Best cycle")} value={fmtMoneySigned(metrics.best_cycle_pnl)}/><Metric label={tr("Найгірший цикл", "Worst cycle")} value={fmtMoneySigned(metrics.worst_cycle_pnl)}/><Metric label={tr("Відсоток перемог", "Win rate")} value={fmtPctSigned(metrics.win_rate_percent)}/>
            </div></section>
            <section className={styles.panel}><h2>{tr("Час і відновлення", "Time & recovery")}</h2><div className={styles.metricGrid}>
              <Metric label={tr("Час у позиції", "Time in position")} value={duration(metrics.time_in_position_seconds, tr)} hint={`${fmtNumber(metrics.time_in_position_percent,1)}% ${tr("тесту", "of test")}`}/><Metric label={tr("Час у відкритому збитку", "Time in open loss")} value={duration(metrics.time_in_loss_seconds, tr)} hint={`${fmtNumber(metrics.time_in_loss_percent,1)}% ${tr("тесту", "of test")}`}/><Metric label={tr("Найдовший негативний період", "Longest negative period")} value={duration(metrics.longest_negative_pnl_period_seconds, tr)}/><Metric label={tr("Найдовша просадка", "Longest drawdown")} value={duration(metrics.longest_drawdown_seconds, tr)}/><Metric label={tr("Відновлення після max DD", "Max DD recovery")} value={metrics.maximum_drawdown_recovered ? duration(metrics.maximum_drawdown_recovery_seconds, tr) : `${tr("Не відновлено", "Not recovered")} · ${duration(metrics.current_drawdown_recovery_seconds, tr)}`}/><Metric label={tr("Середній цикл", "Average cycle")} value={duration(metrics.average_cycle_duration_seconds, tr)}/><Metric label={tr("Медіанний цикл", "Median cycle")} value={duration(metrics.median_cycle_duration_seconds, tr)}/><Metric label={tr("Найдовший цикл", "Longest cycle")} value={duration(metrics.longest_cycle_seconds, tr)}/>
            </div></section>
            <section className={styles.panel}><h2>{tr("Ризик і експозиція", "Risk & exposure")}</h2><div className={styles.metricGrid}>
          ...
          <div className={styles.chartToolbar}>
            ...
            <div className={styles.chartSelects}>
              <select value={cycleFilter} onChange={(e) => { setCycleFilter(e.target.value); setFocusTime(null); setSelectedMarker(null); }}>
                <option value="all">{tr("Усі цикли", "All cycles")}</option>
                {cycles.map((cycle) => <option key={cycle.id} value={cycle.cycle_number}>{tr("Цикл", "Cycle")} #{cycle.cycle_number}</option>)}
              </select>
              ...
              {windowSize === "custom" && <span className={styles.customWindow}>{tr("ВЛАСНИЙ", "CUSTOM")}</span>}
              <span className={styles.toolbarDivider}/>
              <button className={styles.iconWindowButton} title={tr("Збільшити", "Zoom in")} onClick={() => zoomChart(0.65)}><Plus size={14}/></button>
              <button className={styles.iconWindowButton} title={tr("Зменшити", "Zoom out")} onClick={() => zoomChart(1.5)}><Minus size={14}/></button>
              <button className={styles.iconWindowButton} title={tr("Скинути графік", "Reset chart")} disabled={isFullRange && !focusTime} onClick={resetChartView}><RotateCcw size={14}/></button>
            </div>
          </div>
          {filteredPoints.length ? <>
            <div className={styles.chartSnapshot}>
              <div><span>{tr("Видимий період", "Visible period")}</span><strong>{formatPeriodTime(periodStart, chartSpanMs, locale)} — {formatPeriodTime(periodEnd, chartSpanMs, locale)}</strong></div>
              <div><span>{tr("Остання ціна", "Last price")}</span><strong>{fmtMoney(latestPoint?.close)}</strong></div>
              <div><span>Equity</span><strong>{fmtMoney(latestPoint?.equity)}</strong></div>
              <div><span>{tr("Позиція", "Position")}</span><strong>{fmtNumber(latestPoint?.position_qty, 6)}</strong></div>
              <div><span>{tr("Просадка", "Drawdown")}</span><strong className={asNumber(latestPoint?.drawdown_percent) < 0 ? styles.snapshotNegative : ""}>{formatAxisPercent(latestPoint?.drawdown_percent)}</strong></div>
            </div>
            <div className={styles.chartHint}><MoveHorizontal size={13}/> {tr("Колесо — масштаб навколо курсора · перетягування — панорамування · Shift + колесо — горизонтальне переміщення · подвійний клік — скидання. Синя смуга показує час у позиції, червона — відкритий збиток, маркери — виконання ордерів.", "Scroll to zoom around the pointer · drag to pan · Shift + scroll to move horizontally · double-click to reset. The blue strip is position time, the red strip is open loss, and markers show fills.")}</div>
            {selectedMarker && <div className={styles.markerDetails}>
              <div className={styles.markerDetailsHeader}>
                <div><span>{selectedMarker.count > 1 ? tr("Кластер виконань", "Fill cluster") : selectedMarker.kind === "cancelled" ? tr("Скасований ордер", "Cancelled order") : tr("Виконання", "Execution")}</span><strong>{selectedMarker.label}</strong></div>
                <button onClick={() => setSelectedMarker(null)} title={tr("Закрити деталі", "Close details")}><X size={16}/></button>
              </div>
              ...
              {selectedMarker.clusterItems?.length > 1 && <div className={styles.clusterList}>{selectedMarker.clusterItems.slice(0, 8).map((item) => <button key={item.id} onClick={() => { setSelectedMarker(item); setFocusTime(item.timestamp); setClampedViewRange(item.timestamp - (DAY_MS / 2), item.timestamp + (DAY_MS / 2), "custom"); }}><span>{new Date(item.timestamp).toLocaleDateString(locale)}</span><strong>{item.label}</strong><em>{fmtMoney(item.price)}</em></button>)}{selectedMarker.clusterItems.length > 8 && <small>+{selectedMarker.clusterItems.length - 8} {tr("виконань — збільште масштаб, щоб розділити їх", "more fills — zoom in to separate them")}</small>}</div>}
            </div>}
            <div
            ...
            <div className={styles.navigatorChart}>
              <div className={styles.navigatorHeader}><span>{tr("Навігатор таймлайна", "Timeline navigator")}</span><strong>{formatPeriodTime(periodStart, chartSpanMs, locale)} — {formatPeriodTime(periodEnd, chartSpanMs, locale)}</strong></div>
              <ResponsiveContainer width="100%" height={82}>
                <AreaChart data={cycleBasePoints} margin={{ top: 4, right: 8, left: 8, bottom: 2 }}>
                  ...
                  <Brush
                    ...
                    tickFormatter={(value) => formatChartTime(value, fullRangeSpan, locale)}
                    stroke="#25c78b"
                    fill="#0a0d14"
                  />
                </AreaChart>
            ...
            <div className={styles.subCharts}>
              <MiniChart title="Equity" value={fmtMoney(latestPoint?.equity)} data={filteredPoints} dataKey="equity" stroke="#25c78b" fill="rgba(37,199,139,.12)" domain={equityDomain} timeDomain={timeDomain} timeTicks={miniTimeTicks} yFormatter={(value) => formatAxisMoney(value, locale)} tooltipFormatter={fmtMoney} spanMs={chartSpanMs} referenceValue={asNumber(run.initial_balance ?? normalizedPoints[0]?.equity)} referenceLabel={tr("Старт", "Start")}/>
              <MiniChart title={tr("Просадка", "Drawdown")} value={formatAxisPercent(latestPoint?.drawdown_percent)} data={filteredPoints} dataKey="drawdown_percent" stroke="#f05b63" fill="rgba(240,91,99,.12)" domain={drawdownDomain} timeDomain={timeDomain} timeTicks={miniTimeTicks} yFormatter={formatAxisPercent} tooltipFormatter={formatAxisPercent} spanMs={chartSpanMs} referenceValue={0}/>
              <MiniChart title={tr("Розмір позиції", "Position size")} value={fmtNumber(latestPoint?.position_qty, 6)} data={filteredPoints} dataKey="position_qty" stroke="#4d9dff" fill="rgba(77,157,255,.12)" domain={positionDomain} timeDomain={timeDomain} timeTicks={miniTimeTicks} yFormatter={(value) => formatAxisQty(value, locale)} tooltipFormatter={(v) => fmtNumber(v, 6)} spanMs={chartSpanMs} type="stepAfter" referenceValue={0}/>
              <MiniChart title={tr("Доступний баланс", "Available balance")} value={fmtMoney(latestPoint?.available_balance)} data={filteredPoints} dataKey="available_balance" stroke="#ffc24b" fill="rgba(255,194,75,.12)" domain={balanceDomain} timeDomain={timeDomain} timeTicks={miniTimeTicks} yFormatter={(value) => formatAxisMoney(value, locale)} tooltipFormatter={fmtMoney} spanMs={chartSpanMs}/>
            </div>
        ...
        {tab === "Configuration" && <div className={styles.configGrid}><section className={styles.panel}><h2>{tr("Знімок бота", "Bot snapshot")}</h2><Config data={run.bot_snapshot}/></section><section className={styles.panel}><h2>{tr("Конфігурація backtest", "Backtest configuration")}</h2><Config data={run.configuration}/></section><section className={styles.panel}><h2>{tr("Технічні дані", "Technical")}</h2><Config data={{run_id:run.id,temp_bot_id:run.temp_bot_id,emulator_account_id:run.emulator_account_id,created_at:run.created_at,started_at:run.started_at,completed_at:run.completed_at,candles:run.total_candles}}/></section></div>}
      </div>
    </main>
  );
}
