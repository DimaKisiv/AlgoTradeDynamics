  ...
  return (
    <main className="run-detail">
      <div className="container">
        ...
        {loading && <div className="run-detail__state">Завантаження…</div>}
        {error && <div className="run-detail__state run-detail__state--error">⚠ {error}</div>}
        {run && (
          <>
...
function ParamsBlock({ run }) {
  return (
    <div className="run-detail__params glass">
...
function ParamGroup({ title, entries }) {
  return (
    <div className="param-group">
      <p className="param-group__title">{title}</p>
      <dl className="param-group__list">
        {Object.entries(entries).map(([k, v]) => (
          <div key={k} className="param-group__item">
            <dt>{k}</dt>
